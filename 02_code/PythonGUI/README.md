...
- **测序文件 (FASTQ)**、**目的序列 (FASTA)**：文件选择器；若 FASTQ 所在目录中存在
  ...
  复制出去即可直接用。
- **输出目录**：默认 `我的文档\nanoamp 结果`（可从「浏览…」更改）。
- **分析名称**（可选，与输出目录同一行）：给这次分析起个名字，留空则用开始时间
  （`2026-09-27 10:24:31`）。名字只用于窗口内的标注——状态栏、运行日志、
  「查看上次结果」——**不传给 R，也不写入输出文件**。
...
- **六个标签页**：单倍型结果（选中一行显示完整序列）、注释结果、变异注释、
  ...
  每页都有**水平滚动条**；表格列宽固定、只有最后一列随窗口伸缩，因此窗口变窄时横向
  滚动而不是把列挤扁。「单倍型结果」页用 `ttk.Panedwindow` 把表格与序列框分开，
  **分隔线可以拖动**。
- **联动与筛选**：在「单倍型结果」选中一行，注释/变异两页自动只显示该单倍型，
  点「显示全部」恢复；选中注释行会选中对应的单倍型，并可「查看蛋白序列…」
  （或双击）打开可滚动、可复制的蛋白窗口。联动筛选会重画注释表，重画后**原先选中的
  行仍然选中**，所以按钮与双击都能直接打开蛋白窗口（此前会误报"请先选中一行"）。
- **QC 指标说明**：QC 页的「指标说明」勾选框在右侧列出 `qc.tsv` 里每个指标的含义
  （三种模式的专属指标 + 功能注释追加的指标）。它与是否跑过分析无关。
...
- **查看上次结果**：点「开始分析」后**每个结果页都会清空**，上一次运行的内容留在内存里
  ...
  状态行与各页标出那次的名称与时间。运行日志**不清空**，它是本次会话的历史。
  注释两页只显示**本次运行**产生的内容：R 不会删除上一次的 `annotation.tsv`，因此
  本次没开注释时，窗口会说明"输出目录里还有上一次留下的文件，未显示"，而不是把旧结果
  连同空的计数（"已注释 个转录本…"）当成新结果画出来。变异注释页还会在没出结果时
  指出缺哪一步（未开功能注释 / 未勾选输出变异级明细）。

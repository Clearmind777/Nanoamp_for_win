...
给**要在 R 里写代码、或把 nanoamp 嵌进自己分析流程**的同学。

如果你只是想跑一个样本看结果，用图形界面版更省事 —— 见
...
> 如果不想编译，用 `install.exe` 装好的二进制依赖库更省事：
> 库位置是 `%LOCALAPPDATA%\nanoamp\R\lib`。
...
| `aligner` | `"minimap2"` | 换成 `"r"` 使用 R 内成对比对（无需外部程序，较慢） |
| `threads` | 4 | 线程数 |

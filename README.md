...
- 已有 **R 4.6.x** → 安装器直接用它，不会再装一份 R；
- 只有 **R 4.5 或别的版本** → 安装器会安装**随包提供的 R 4.6**，装在
  `<安装目录>\R\R-runtime`，**不会改动、也不会卸载你原有的 R**。
  装完 `nanoamp doctor` 里的 `R version` 应当是 4.6.x。

> 0.1.2 及更早的安装器在这台机器上会失败：它只要找到 R ≥ 4.2 就用，
> 于是选中 R 4.5，随后因为包里只有 4.6 的依赖包而报
> 「安装包内没有适配 R 4.5 的依赖包」。**0.1.3 起已修复**（见
> `00_materials/work_reports/work_report.14.md`）。
>
> 如果你手上还是 0.1.2 的安装包，两条临时办法：

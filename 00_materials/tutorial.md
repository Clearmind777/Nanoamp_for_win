...
| 步骤 | 内容 |
|---|---|
| 1 | 找到或安装 R：系统里已有 R 且**版本与包内依赖包一致（当前 4.6）**时直接用；不一致（比如只有 R 4.5）就安装随包提供的 R 4.6，装在安装目录里，**不动你原有的 R** |
...
7. **装不上、说 R 版本不对**：nanoamp 的 109 个依赖包是按**某一个** R 小版本编译的
   ...
   - 如果安装时看到「安装包内没有适配 R x.y 的依赖包」而退出，说明安装包不完整
     （缺 `_offline/r/R-*-win.exe`），重新完整解压两个附件再装。

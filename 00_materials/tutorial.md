...
卸载：双击**安装目录里的 `uninstall.exe`**（安装时安装器就把安装包里的那份复制了进去，
所以装完可以删掉解压出来的安装包；`release\uninstall.exe` 同样可用，是同一个程序）。
它自己就在安装目录里时，会先删掉其余内容、再在窗口关闭后自动删除自身与空目录。
**你自己装的 R、你的数据和结果文件都不会被删除。**
...
| **输出目录** | 结果放哪。默认 `我的文档\nanoamp 结果`，一般不用改 |
| **分析名称** | 可选，与输出目录同一行。给这次分析起个名字（例如"样本 A 复测"），留空就用开始时间。只影响窗口里的标注（状态栏 / 日志 /「查看上次结果」），**不参与分析** |
| **模式** | 保持 `A - 参考引导（推荐）` |
...
| **CDS 起 / 止 / 链 / 读码框** | 仅「离线 CDS」路线要填：CDS 在**目的序列**上的起止（1-based，两端都算）、链、读码框。选好目的序列后，「止」会按该序列长度预填（提示行标明是预填值，可改），这只是起步值，不是真实 CDS 的终点 |
...
| **单倍型结果** 标签页 | 核心结果表。选中一行，下方显示这条序列的完整碱基；**两栏之间的分隔线可以用鼠标上下拖动**，想看长序列就往下拉 |
| **注释结果** 标签页 | 每个「单倍型 × 转录本」一行的后果（仅在启用注释时有内容）；选中某个单倍型后这张表自动只显示该单倍型，点「显示全部」恢复 |
| **变异注释** 标签页 | 每个变异一行的后果（仅在启用注释且勾选明细时有内容），同样跟随单倍型筛选；没出结果时会说明缺哪一步（未开功能注释 / 未勾明细） |
| **查看蛋白序列…** 按钮 | 打开一个可滚动、可复制的窗口显示选中行的参考/突变蛋白（需要勾选「输出蛋白序列」重跑）。联动筛选重画表格后，你点中的那一行**仍然保持选中**，按钮与双击都能直接打开 |
| **QC 指标** 标签页 | 数据质量。勾选左上角**「指标说明」**会在右侧列出每个指标的含义，**不需要先跑分析** |
| **输出文件** 标签页 | 本次生成的文件的清单，大小按 `KB`/`MB`/`GB` 显示，双击可用系统默认程序打开 |
| **运行日志** 标签页 | R 的实时输出，**出错时看这里** |
| **开始分析** 按钮 | 启动分析（后台线程，窗口不会卡死）。点它会把六个结果页里上一次运行的内容**清空**；「运行日志」保留 |
| **查看上次结果** 按钮 | 回看被清空的上一次运行（内容留在内存里，关掉程序即消失），状态行标出那次的名称与时间；再点一次变成「返回本次结果」 |
| **环境自检** 按钮 | 跑一次 `nanoamp doctor`，把 R / 依赖 / minimap2 / curl / 缓存目录 / 配置目录 / 注释可用性的状态写进日志 |
| **打开输出目录** 按钮 | 在资源管理器里打开结果文件夹 |

> 注释两页只显示**本次运行**产生的内容。若输出目录里还留着上一次的 `annotation.tsv`，
> 状态行会写"输出目录里还有上一次运行留下的 …，未显示"，不会把旧结果当成新结果。
>
...
- 安装版的做法是：生成的 CLI 驱动脚本里**直接写死** `NANOAMP_MINIMAP2`
  ...
  图形界面同样会设置这个变量：它先找到安装目录（默认 `%LOCALAPPDATA%\nanoamp`，
  换过安装位置时由 `%LOCALAPPDATA%\nanoamp.path` 指向），再把 `<安装目录>\bin\minimap2.exe`
  交给 R —— 所以**安装时没有勾选"加入 PATH"也能用**。窗口启动时会把实际路径写进运行日志。

...
- 结果就是"一字不差"的 read 只占一部分。本教程用的 E4-3 样本是 **31.5%**，
  别的样本可能只有百分之几 —— 差别取决于测序质量、序列长度和真实混样比例。

**这不是实验失败，是测序错误把真实结果掩盖了。**

nanoamp 的做法是：先比对 → 找出候选变异 → 把不满足可信度门槛的差异**当成测序错误改掉** → 再按"校正后的序列"分类计数。

所以 nanoamp 给出的 33.3% / 31.5% / 26.8% 才是可信的比例，而 `exact_reference_proportion`
（原始、未校正的精确匹配率，见 §4.4）只是给你一个对照。
...
| `exact_reference_proportion` | 0.3146 | 原始 reads 中一位不差的比例 |

> **注意最后一行**：`exact_reference_proportion` 是**校正前**的原始匹配率。
> 本样本恰好是 31.5%，看起来和 H2 一样，但那是巧合（因为 218delG 占了六成，
> 真正"一位不差"的原始 read 恰好主要来自正确的那一类）。
> 在别的样本上这个数字常常只有 **2%–4%**。读它的时候记住：**它不是最终结论，
> 只是"测序错误有多严重"的体检指标。**
>
> 一句话：`exact_reference_proportion` 低 ≠ 实验失败；`is_reference = 是` 的占比才是答案。
...
> 一个样本失败**不会**中断整批。跑完看 `status` 列，哪个是 `error` 就去对应的
> 子目录 / 看 `error` 列的内容。

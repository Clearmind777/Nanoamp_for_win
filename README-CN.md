...
「输出变异级明细」这一勾选框属于「功能注释…」面板，勾选「功能注释…」后才会出现；
没出结果时「变异注释」页会说明缺哪一步。
...
回看，回看时标出那次的**名称与时间**；直到关闭程序。窗口启动时会往日志里写
「仓库根目录 / Rscript / minimap2」三行实际路径。

图形界面还有这些便于查看的细节：输入区可以填**分析名称**（留空则用开始时间）用来区分
两次运行；「单倍型结果」页表格与序列框之间的分隔线可用鼠标上下拖动；每个结果页都有
**水平滚动条**（列宽固定，窗口变窄时横向滚动）；「输出文件」页的大小按 KB/MB/GB 显示；
「QC 指标」页勾选**「指标说明」**会在右侧列出每个指标的含义（与是否跑过分析无关）。

安装器会把 **`uninstall.exe` 复制到安装目录**：装完即可删除解压出来的安装包，卸载双击
安装目录里的那一个即可（它删完其余内容后会在窗口关闭时删除自身与空目录）。

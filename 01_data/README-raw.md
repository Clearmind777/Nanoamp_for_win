...
> 本文档描述**公司交付时的原始文件名与目录结构**。原始交付目录不再以
> `01_data/test_data/` 的形式存放：`TSM20260826/`、`ZNF8/`、`nano_seq/` 三个数据集
> 已改为 `<dataset>/<sample>/<规范化文件名>`（`reads.fastq`、`reference.self.fa`、
> `consensus.N.fa`、`variants.N.xlsx`、`sanger.N.ab1`），
> **每个样本目录中的 `meta.tsv` 记录了规范化文件名与原文件名的对应关系**。
> `SD260728184122_1/`、`SD260812174403_1/` 两个批次按公司原始结构保留。
> 下文出现的 `test_data/` 路径指当初的原始交付目录。
...
> 说明：本文根据目录结构、文件内容和 BAM 头信息整理。具体基因、编辑方式和样本分组需与实验人员核对。
...
| `*.var.xls` | 实际为 TSV 文本 | 全部变异位点的完整表，包含 PASS 和 FILTERED | 文本编辑器、`column`、Python、Excel |
| `*.filt.var.xls` | 实际为 TSV 文本 | 过滤后的变异表，一般只保留通过过滤的位点 | 同上 |
...
| `*.fai` | FASTA 索引 | 记录参考序列长度和偏移，供 samtools 快速访问 | 自动生成，一般无需手动查看 |
...
该目录的比对模式：reads 比对到公共扩增子参考 `for.ref`，变异相对参考序列计算，因此 `*.var.xls` 中的 `PASS` 位点可直接视为候选编辑位点。
...
该目录的测序深度差异较大：`G22608076118` 只有 225 条比对 reads，而 `G22608076105` 有 14,005 条。低深度样本的变异频率可靠性会明显低于高深度样本。
...
该目录的 BAM 参考序列是该样本自己的共识序列，不是公共参考基因组。因此：
...
`fastq.fai` 是按 FASTA 索引方式为 FASTQ 建立的索引，属于流程中间产物，不是标准 FASTQ 文件。
...
扩展名为 `.xls`，但这两个文件通常是制表符分隔的文本（TSV），可用文本编辑器或 `column -t -s$'\t'` 查看。
...
1. **`.var.xls` 不是真正的 Excel 文件**
   - 单样本 `*.var.xls` 和 `*.filt.var.xls` 实际是制表符分隔的文本，直接用 Excel 打开可能触发格式警告。
...
2. **`merged_data.var.xls` 是真正的 Excel 二进制文件**
...
5. **一个样本可能有多个簇结果**
   ...
   - 下结论前需确认哪个簇是主导簇，以及各簇的 reads 支持量。
...
如需进一步整理，建议补充一份实验设计表，将样本与基因、靶点、处理条件和预期编辑结果对应起来。
